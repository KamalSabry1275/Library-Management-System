...
import { plusModule } from "../rtk/slices/modulesSlice";
import { NewModule } from "../pages/EditorProject";
...
export function Drag_Drop() {
  ...
  useEffect(() => {
    ...
    function addElement(project_page, y) {
      let sensors = [
        ...project_page.querySelectorAll(
          ".project_page .sensor:not(.dragging)"
        ),
      ];
    ...
    return () => {
      newSensors?.forEach((newSensor) => {
        newSensor.removeEventListener("dragstart", (e) => {
          newSensor.classList.add("dragging");
        });
        newSensor.removeEventListener("dragend", (e) => {
          newSensor.classList.remove("dragging");
        });
      });

      sensors?.forEach((sensor) => {
        sensor.removeEventListener("dragstart", (e) => {
          sensor.classList.add("dragging");
          newSensor = sensor.cloneNode(true);
        });
        sensor.removeEventListener("dragend", (e) => {
          sensor.classList.remove("dragging");
          // add sensor in modules in modules reducer
          console.log(newSensor);
      ...
      //   if (newItem != null) container.removeChild(newItem);
      // });
      project_page?.removeEventListener("dragover", (e) => {
        e.preventDefault();
